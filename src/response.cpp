...
size_t Response::response_to(int client_fd, const std::string &web_root_path, const std::string &request_path)
{
    ...
    if (validate(response_path))
    {
        ...
        if (content_len == dir_size)
        {
            ...
            return response_dir_to(client_fd, response_path, request_path);
        }
...
size_t Response::response_dir_to(int client_fd, const std::string &dir_path, const std::string &request_path)
{
    ...
    else
    {
        ...
        while ((dirp = readdir(dp)) != NULL)
        {
            ...
            if (dirp->d_name[0] == '.')
                continue;
            std::string file_path = dir_path + "/" + dirp->d_name;
